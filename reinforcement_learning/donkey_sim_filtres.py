...
class DonkeyUnitySimHandler(IMesgHandler):
    def __init__(self, conf: Dict[str, Any]):
        ...
        self.line_position_history = deque(maxlen=5)  # Adjust maxlen as needed
        self.previous_distance = None
        self.previous_sign = None
        self.continuity_threshold = 70
    ...
    def calculate_distance(self):
        ...
        TRESH3_MIN = np.array([0, 0, 0],np.uint8)
        TRESH3_MAX = np.array([120, 130, 180],np.uint8)

        image=255 - cv2.inRange(image, TRESH3_MIN, TRESH3_MAX)
        image=cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

        y_ref = image.shape[0] - 10
        reference_x_value = 80

        # Scan horizontally to find the white pixels at y_ref
        horizontal_line = image[y_ref, :]
        white_pixels_indices = np.where(horizontal_line == 255)[0]

        # If no white pixels are found, we cannot calculate the distance
        if white_pixels_indices.size == 0:
            return float("inf")
            print("No white line detected at the reference Y-coordinate.")
        else:
            # Find the white pixel closest to the reference X-coordinate
            closest_white_pixel_x = white_pixels_indices[np.argmin(white_pixels_indices - reference_x_value)]
            
            # Calculate the distance from the reference X-coordinate
            distance_from_reference_x = closest_white_pixel_x - reference_x_value
            return distance_from_reference_x
    ...
    def break_line(self):
        ...
        if distance_from_reference_x!=float("inf"):
            ...
            if self.line_position_history and not np.isclose(distance_from_reference_x, self.line_position_history[-1], atol=self.continuity_threshold):
                pass
                #print("Detected line is discontinuous with the history. It might be a different line.")
            else:
    ...
    def determine_episode_over(self):
        ...
        if math.fabs(self.cte) > 2 * self.max_cte:
            ...
            self.over = True
        crossed = self.break_line() 
        if crossed and self.n_steps>15:
            print("crossed")
            self.over = True
        ...
        if abs(self.speed) < self.min_speed and self.n_steps > 40:
